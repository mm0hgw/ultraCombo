...
is.ultraCombo <- function(x){
	if(!(inherits(x,"ultraCombo")))return(FALSE)
	if(!is.valid.nk(x$n,x$k))return(FALSE)
	if(!is.valid.index(x$i,x$n,x$k))return(FALSE)
	return(TRUE)
}
...
#	debugCat("is.valid.nk:",n,k,"\n")
	error_list<-vector()
	if(length(n)!=1){
		error_list<-c(error_list,paste("n argument is invalid length",paste(n,collapse=",")))
	}
	if(length(k)!=1){
		error_list<-c(error_list,paste("k argument is invalid length",paste(k,collapse=",")))
	}
	if(sum(n%%1!=0)>0){
		
		error_list<-c(error_list,paste("non integer n argument:",paste(n[n%%1!=0],collapse=",")))
	}
	if(sum(k%%1!=0)>0){
		error_list<-c(error_list,paste("non integer k argument:",paste(k[k%%1!=0],collapse=",")))
	}
...
#	debugCat("is.valid.combination",n,paste(x,collapse=","))
	if(length(dim(x))>1){
		lapply(seq(nrow(x)),
			function(y)is.valid.combination(x[y,],n)
		)
		return(3)
	}else{
		error_list<-vector()
		errors<-x[x%%1!=0]
		if(length(errors)>0){
			error_list<-c(error_list,paste("non integer x input:",paste(errors,collapse=":")))
		}
		errors<-c(x[(x<1)],x[(x>n)])
		if(length(errors)>0){
			error_list<-c(error_list,paste("x values out of range:",paste(errors,collapse=":")))
		}
		errors<-x[duplicated(x)]
		if(length(errors)>0){
			error_list<-c(error_list,paste("duplicate x values",paste(errors,collapse=":")))
		}
		if(length(error_list)>0){
			stop(paste(error_list,collapse=":"))
		}else{
